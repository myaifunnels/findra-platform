...
function Card({ icon, kicker, title, copy, status, enabled, onToggle, toggling, children }) {
  const label = statusLabel(status);
  const on = label === "Connected";
  return (
    <article className={`findra-int-card ${on ? "is-on" : ""}`}>
      <header className="findra-int-head">
        <span className="findra-int-icon">{icon}</span>
        <div>
          <small>{kicker}</small>
          <h3>{title}</h3>
          <p>{copy}</p>
...
export function IntegrationsAdmin({ onNotify, embedded = false }) {
  return (
    <div className={embedded ? "findra-integrations embedded" : "admin-content findra-integrations"}>
      {!embedded && (
        <section className="welcome-row">
          <div>
            <span className="section-eyebrow">Workspace</span>
            <h2>Integrations</h2>
            <p>Connect the services Findra uses. Keys stay on the server and are never shown in full.</p>
          </div>
        </section>
      )}
...
function PayMongoIntegration({ onNotify }) {
  ...
  return (
    <Card icon={<CreditCard weight="duotone" />} kicker="Payments" title="PayMongo" copy="Cards, GCash, QR Ph, and online banking." status={status} enabled={status.enabled} toggling={busy === "toggle"} onToggle={toggle}>
...
function BrevoIntegration({ onNotify }) {
  ...
  return (
    <Card icon={<EnvelopeSimple weight="duotone" />} kicker="Email" title="Brevo" copy="Transactional mail, newsletter signups, and sender identity." status={status} enabled={status.enabled} toggling={busy === "toggle"} onToggle={toggle}>
...
function GoogleMapsIntegration({ onNotify }) {
  ...
  return (
    <Card icon={<MapPin weight="duotone" />} kicker="Location" title="Google Maps" copy="Address search on listing forms and public maps." status={status}>
...
function R2Integration({ onNotify }) {
  ...
  return (
    <Card icon={<Cloud weight="duotone" />} kicker="Files" title="Cloudflare R2" copy="Logos, galleries, videos, and brochures." status={status} enabled={status.enabled} toggling={busy === "toggle"} onToggle={toggle}>
...
function TextBeeIntegration({ onNotify }) {
  ...
  return (
    <Card icon={<Phone weight="duotone" />} kicker="SMS" title="TextBee" copy="Optional SMS next to Brevo email." status={status}>
